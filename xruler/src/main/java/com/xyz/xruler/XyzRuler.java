...
public class XyzRuler extends View implements ValueAnimator.AnimatorListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int textColor = ta.getColor(R.styleable.XyzRuler_rTextColor, Color.BLUE);
    ...
    private void drawScale(Canvas canvas) {
        ...
        for (int x = 0; x < mWidth; x++) {
            int y=mHeight/2;
            IndexY = startY + lineHeight;
            boolean isDrawText = false;
            boolean isDrawText5 = false;

            if ((-sumMoveX + x) % (pixel * 5) == 0) {//到五刻度的时候画较长的刻度
                IndexY += lineHeight;
                isDrawText5 = true;
            }
            if ((-sumMoveX + x) % (pixel * 10) == 0) {//到十刻度的时候画最长的刻度
                IndexY +=2*lineWidth;
                isDrawText = true;
            }
...
//                canvas.drawLine(x,  mHeight, x, mHeight - IndexY, lPaint);
                canvas.drawLine(x,  y+IndexY, x, y - IndexY, lPaint);
              /*  System.out.println("起始坐标："+x+"/t"+mHeight);
                System.out.println("终点坐标："+x+"/t"+(mHeight - IndexY));*/
            }
            if(isDrawText5&&!isDrawText){
                if (text >= begin && text <= end) {
//                canvas.drawText(String.valueOf(text), x,  y-2*lineWidth - IndexY - lineToText+2*lineHeight, tPaint);
                    canvas.drawText(String.valueOf(text), x, y + IndexY + 2*lineToText+3*lineWidth, tPaint);
                }

//                mHeight - y - lineToText
//                zhengHgiht=  y - IndexY;
            }

            if (isDrawText) {
                if (text >= begin && text <= end) {
//                    canvas.drawText(String.valueOf(text), x,  y - IndexY - lineToText, tPaint);
                    canvas.drawText(String.valueOf(text), x, y + IndexY + 2*lineToText+2*lineWidth, tPaint);
                    zhengHgiht=  y + IndexY ;
                }
            }
        }
...
//        lPaint.setStrokeWidth(borderWidth);
        canvas.drawLine(halfWidth-4*lineWidth,mHeight/2+2*lineHeight+4*lineWidth, halfWidth-4*lineWidth,mHeight/2-2*lineHeight-3*lineWidth, lPaint);

//        canvas.drawLine(halfWidth-2*lineWidth,  mHeight, halfWidth-2*lineWidth,3*lineHeight+lineWidth  , lPaint);

        canvas.drawLine(halfWidth+4*lineWidth, mHeight/2+2*lineHeight+4*lineWidth, halfWidth+4*lineWidth,mHeight/2-2*lineHeight-3*lineWidth, lPaint);

        canvas.drawLine(halfWidth-4*lineWidth, mHeight/2-2*lineHeight-3*lineWidth, halfWidth+4*lineWidth,mHeight/2-2*lineHeight-3*lineWidth, lPaint);

        canvas.drawLine(halfWidth-4*lineWidth, mHeight/2+2*lineHeight+4*lineWidth, halfWidth+4*lineWidth, mHeight/2+2*lineHeight+4*lineWidth, lPaint);
    }
